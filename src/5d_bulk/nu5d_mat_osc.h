...
#define DMSQ21 (7.49e-5)      // [eV^2] NuFit 6.0
#define DMSQ31_NH (2.513e-3)  // [eV^2] NuFit 6.0
#define DMSQ32_IH (-2.484e-3) // [eV^2] NuFit 6.0
...
static int GetModesCutoff() {
    return modes;
}

void SetModesCutoff(int n) {
    modes = n;
}

double coth_eq(double, void*);
double cot_eq(double, void*);

double mum_to_eVinv(double x) { return x * 5.06773; }
double eVinv_to_mum(double x) { return x / 5.06773; }

void make_PMNS(gsl_matrix_complex* U, int cp_sign) {
    double complex(*_U)[GLB_NU_FLAVOURS] = (double complex(*)[GLB_NU_FLAVOURS])gsl_matrix_complex_ptr(U, 0, 0);
    // std::complex<double>(*_U)[GLB_NU_FLAVOURS] =
        // (std::complex<double>(*)[GLB_NU_FLAVOURS])gsl_matrix_complex_ptr(U, 0, 0);
    if (cp_sign > 0) {
        _U[0][0] = cos(th12) * cos(th13);
        _U[0][1] = sin(th12) * cos(th13);
        _U[0][2] = sin(th13) * cexp(-I * deltacp);

        _U[1][0] = -sin(th12) * cos(th23) - cos(th12) * sin(th23) * sin(th13) * cexp(I * deltacp);
        _U[1][1] = cos(th12) * cos(th23) - sin(th12) * sin(th23) * sin(th13) * cexp(I * deltacp);
        _U[1][2] = sin(th23) * cos(th13);

        _U[2][0] = sin(th12) * sin(th23) - cos(th12) * cos(th23) * sin(th13) * cexp(I * deltacp);
        _U[2][1] = -cos(th12) * sin(th23) - sin(th12) * cos(th23) * sin(th13) * cexp(I * deltacp);
    ...
    // 		printf("U[%d, %d] = %f + (%f)i \n", i, j, GSL_REAL(Uij), GSL_IMAG(Uij));
    // 	}
    // }
}

void make_RRH(gsl_matrix_complex* RRH, gsl_matrix_complex* U,
...
// E [GeV], masses [eV], R [micron], density [g/cm^3]
{
    ...
    for (int i = 0; i < 3; i++) {
        if (fabs(cR[i]) < DELTA) {
            ...
            RY[i][0] = muR[i] * sqrt(2 * M_PI * cR[i] / (exp(2 * M_PI * cR[i]) - 1));
        // printf("Ysq[%d][0]=%f\n", i, RY[i][0]);

        for (int n = 1; n < modes; n++) {
            RY[i][n] = muR[i] * n * sqrt(2 / (n * n + cR[i] * cR[i]));
            // printf("Ysq[%d][%d]=%f\n", i, n, RY[i][n]);
        }
    }

    // printf("Y[i][n] has been created.\n");
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int n = 1; n < modes; n++) {
            ...
            gsl_complex B = gsl_complex_rect(RY[i][n] * sqrt(lsq), 0);
...
int prob_matrix_5dnu(double (*P)[3], int cp_sign, const double E, const double length, const double density) {
    ...
    gsl_matrix_complex* exp = gsl_matrix_complex_alloc(3 * modes, 3 * modes);
    ...
    gsl_matrix_complex_set_zero(exp);
    ...
    qsort(eval, 3 * modes, sizeof(double), cmp);
    // for (int i = 0; i < 3 * 3; i++) {
    //     printf("eval[%d] = %2.12f \n", i, eval[i]);
    // }
    ...
    for (int n = 0; n < 3 * modes; n++) {
        gsl_matrix_complex_set(exp, n, n, gsl_complex_polar(1, L_GeVinv * gsl_vector_get(e_val, n) / (2 * E * sq(R_eVinv))));
    }

    gsl_blas_zgemm(CblasNoTrans, CblasNoTrans, GSL_COMPLEX_ONE, UL, exp, GSL_COMPLEX_ZERO, ULexp);
